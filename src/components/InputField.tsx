...
const InputField = ({children, description, disabled, name, placeholder, required, type = 'text'}: Props) => {
    return (
        <div className={'flex flex-col gap-0.5 font-roboto mb-4 w-full'}>
            ...
            <input
                className={
                    'disabled:opacity-70 border-2 rounded-md border-primary/40 hover:border-primary/70 focus:outline-none focus:border-primary placeholder:italic placeholder:text-slate-400 w-full py-3 px-4 bg-white'
                }
                disabled={disabled}
                name={name}
